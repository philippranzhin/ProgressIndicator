...
namespace Components
{
    internal static class ProgressModelExtensions
    {
        ...
        public static ProgressModel<T> Bind<T>(this ProgressModel<T> self) where T : IConvertible
        {
            ...
            void Pause()
            {
                if (self.OperationState == OperationState.Paused) return;

                ChangeState(self.WithOperationState(OperationState.Paused).WithoutSubOperation());
            }

            void Finish()
            {
                if (self.OperationState == OperationState.Finished) return;

                ChangeState(self.WithOperationState(OperationState.Finished).WithoutSubOperation());
            }
        ...
        public static double? Speed<T>(this ProgressModel<T> self) where T : IConvertible
        {
        ...
        public static string ConvertedProgress<T>(this ProgressModel<T> self) where T : IConvertible
        {
        ...
        public static string ConvertedTime<T>(this ProgressModel<T> self) where T : IConvertible
        {
            return self.Config.TimeConverter(self.Time());
        }

        public static TimeSpan? Time<T>(this ProgressModel<T> self) where T : IConvertible
        {
            var speed = self.Speed();
            if (speed != null)
        ...
        public static double Percent<T>(this ProgressModel<T> self) where T : IConvertible
        {
            if (self.Progress <= 0) return 0;
            ...
            where T : IConvertible
        {
            ...
            {
                var speed = (self.Progress - previous.Value.Progress) /
                            (self.Time - previous.Value.Time).TotalMilliseconds;
